...
public class CardImageLoader {
    ...
    private String createFilename(Card card) {
        return card.getSuit().getCode() + "-" + card.getRank().getSymbol() + ".png";
    }

    private Image loadImage(String filename) {
        return imageCache.computeIfAbsent(filename, this::loadUncachedImage);
    }

    private Image loadUncachedImage(String filename) {
        String resourcePath = "/" + filename;

        URL resourceUrl = CardImageLoader.class.getResource(resourcePath);
